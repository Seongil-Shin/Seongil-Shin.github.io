...
- View : UI로직. 사용자 눈에 보이는 인터페이스를 담당함. UI에 연관된 로직만 수행한다. 필요한 데이터는 View Model과 data binding을 통해 얻는다.
- View Model : Model에 데이터를 요청하고 가공함. 비지니스 로직을 처리
...
- 데이터를 관리하는 로직과 UI로직을 깔끔하게 분리하면, 애플리케이션을 보다 쉽게 테스트, 유지보수 할 수 있음.
