...
- 일반적으로 보안관리는 XML 배포 서술자에 기록하므로, 보안에 대해 수정할 일이 생겨도 자바 소스 코드를 수정하여 다시 컴파일 하.지 않아도 보안관리가 가능합니다.
...
이 과정에서 JVM은 클라이언트의 요청들을 각각 불리된 스레드 내부에서 처리한다. 즉, 서블릿을 사용하는 것은 JVM이 각 요청을 분리된 자바 스레드 내부에서 처리하도록 하는 것이다.  각 서블릿은 HTTP 요청에 응답하는 특정한 요소들이 있는 자바 클래스이다. 

> 대부분의 사례에 서블릿 컨테이너는 하나의 JVM에서 동작하느지만 컨테이너가 여러 개의 JVM들을 필요로하는 문제들이 존재하기도 한다.
