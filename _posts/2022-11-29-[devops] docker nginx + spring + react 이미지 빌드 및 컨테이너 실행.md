...
최상단 디렉터리에 `Dockerfile`과 nginx 설정에 필요한 `nginx.conf`를 배치해두었다. `startApp.sh`는 컨테이너가 실행될 때 스프링과 nginx를 실행할 스크립프 파일이다. React와 Spring boot는 각각 client, server 디렉터리에 배치하였다.
...
- FROM : spring boot 빌드를 위해 `gradle:7.4`를 base image로 삼았다. 
...
docker에서는 이미지는 여러 레이어로 구성되어있다. `FROM`,  `RUN`, `COPY`, `ADD` 등의 커맨드를 기준으로 레이어를 구성한다. 레이어는 용량을 최소화하기 위한 것으로, 이미지를 빌드할 때 변경되지 않은 레이어는 기존에 저장된 데이터를 사용하고, 변경된 레이어는 그 레이어부터 상위 레이어까지 새로 명령어를 실행한다.

![docker layer](https://subicura.com/assets/article_images/2017-01-19-docker-guide-for-beginners-1/image-layer.png)

이와 같은 동작으로, COPY, RUN 커맨드를 사용할 땐 자주 변경되지 않는 것은 가능한 상단에 두어 낮은 레이어에 위치시키면, 이미지 빌드 속도를 크게 개선할 수 있다.

따라서 위 react 빌드 dockerfile에서는 상대적으로 자주 변경되지않는 package.json 과 같은 의존성 관리 파일을 상단에 두어 먼저 설치하고, 상대적으로 자주 변경되는 소스파일을 복사하는 COPY 명령어을 후순위에 둔 것이다.
